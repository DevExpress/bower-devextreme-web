...
if (!DevExpress.MOD_VIZ_GAUGES) {
    ...
    (function(DX, $, undefined) {
        ...
        viz.gauges.dxGauge = viz.gauges.dxBaseGauge.inherit({
            ...
            _prepareScaleSettings: function() {
                ...
                scaleOptions.minorTick.customTickValues !== undefined && (scaleOptions.customMinorTicks = scaleOptions.minorTick.customTickValues);
                scaleOptions.minorTick.tickInterval !== undefined && (scaleOptions.minorTickInterval = scaleOptions.minorTick.tickInterval);
    ...
    (function(DX, $, undefined) {
        var viz = DX.viz,
            ...
            SHIFT_ANGLE = 90,
            PI = Math.PI;
        function measureScale(radius, options, textParams) {
            var majorTick = options.tick,
                majorTickEnabled = majorTick.visible && majorTick.length > 0 && majorTick.width > 0,
                minorTick = options.minorTick,
            ...
            if (options.orientation === "inside") {
                ...
                innerCoef = outerCoef = 0.5;
            if (majorTickEnabled) {
                result.min = _min(result.min, radius - innerCoef * majorTick.length);
                result.max = _max(result.max, radius + outerCoef * majorTick.length)
            }
            if (minorTickEnabled) {
                result.min = _min(result.min, radius - innerCoef * minorTick.length);
                result.max = _max(result.max, radius + outerCoef * minorTick.length)
            }
            if (label.visible) {
                if (indentFromTick >= 0) {
        ...
        registerComponent("dxCircularGauge", viz.gauges, viz.gauges.dxGauge.inherit({
            ...
            _measureMainElements: function(elements) {
                ...
                _each(elements.concat(scale), function(_, element) {
                    var bounds = element.measure ? element.measure({radius: radius - element.getOffset()}) : measureScale(radius, scale.getOptions(), scale.measureLabels());
    ...
    (function(DX, $, undefined) {
        ...
        registerComponent("dxLinearGauge", viz.gauges, viz.gauges.dxGauge.inherit({
            ...
            _updateScaleTickIndent: function(scaleOptions) {
                ...
                if (indentFromTick > 0)
                    scaleOptions.label.indentFromAxis = -(indentFromTick + length + textParams[scaleOptions.isHorizontal ? "height" : "width"] / 2);
                else
            ...
            _measureScale: function(layout) {
                var that = this,
                    options = that._scale.getOptions(),
                    majorTick = options.tick,
                    majorTickEnabled = majorTick.visible && majorTick.length > 0 && majorTick.width > 0,
                    minorTick = options.minorTick,
                    minorTickEnabled = minorTick.visible && minorTick.length > 0 && minorTick.width > 0,
                    label = options.label,
                ...
                if (vertical) {
                    ...
                    innerCoef = outerCoef = 0.5;
                if (majorTickEnabled) {
                    result.min = _min(result.min, value - innerCoef * majorTick.length);
                    result.max = _max(result.max, value + outerCoef * majorTick.length)
                }
                if (minorTickEnabled) {
                    result.min = _min(result.min, value - innerCoef * minorTick.length);
                    result.max = _max(result.max, value + outerCoef * minorTick.length)
                }
                if (label.visible) {
                    if (indentFromTick >= 0)
